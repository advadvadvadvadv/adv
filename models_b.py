...
class ResNet110(nn.Module):
	def __init__(self, datashape, n_classes, n_blocks = 18, n_filters = 16, batchnorm = True, bias = False, timestep = 1, return_residus = True, rce = False):
	...
	def forward(self, x):
		...
		x = x.view(x.size(0), -1)
		x = self.classifier(x)
		out = - x if self.rce else x
		return (out, rs) if self.return_residus else out
	def norms_cosines(self, x): # only one image per batch
		x, rs = self.forward_conv(x)
		norms = [torch.mean(r ** 2, (1,2,3))[0] for r in rs]
		cosines = [functional.cosine_similarity(torch.ones(r.size()[1:]).flatten().to(device), r[0,:,:,:].flatten(), dim = 0) for r in rs]
		return torch.stack(norms + cosines)
	def feature_list(self, x):
		...
		x = self.avgpool(x)
		x = x.view(x.size(0), -1)
		return - self.classifier(x) if self.rce else self.classifier(x), x_list
...
class WideResNet(nn.Module):
	def __init__(self, datashape, n_classes, n_blocks = 4, batchnorm = True, bias = False, timestep = 1, return_residus = True, rce = False):
...
class ResNext50(nn.Module):
	def __init__(self, datashape, n_classes, n_blocks = [3, 4, 6, 3], in_filters = 64, planes = 64, expansion = 4, cardinality = 32, width = 4, base = 64, return_residus = True, rce = False):
