...
def adaptive1(classifier_detector, classifier_detector_attack, model, classifier, detector, testloader, n_classes, maha_params, batchsize):
	...
	print('------ Initial attack', attack_name, flush = True)
	for j, (x, y) in enumerate(testloader):
		print('--- image', j + 1)
		n_images += 1
		clean_x, y = x.to(device).float(), y.to(device)
		...
		if clean_correct and not adver_correct:
			...
			if not adver_detected:
				print('successeful undetected classifier attack')
				n_successfull_undetected_simple_attacks += 1
			else:
				print('successeful detected classifier attack, trying classifier detector attack')
				n_successfull_detected_simple_attacks += 1
				one_hot = np.zeros((batchsize, n_classes + 1))
				for i in range(batchsize):
					one_hot[i, adver_pred.item()] = 1
				adver2_x = torch.from_numpy(classifier_detector_attack.generate(x = adver_x.detach().cpu().numpy(), y = one_hot)).to(device, dtype = torch.float)
			...
			print('failed classifier attack')
		if (j + 1) % 10 == 0:
...
def experiment(dataset, modelname, traintype, setting, attack_names, cutoff, correct_on_noisy_only, batchsize, trainsize, valsize, testsize, seed):
	...
	print('Experiment from adv29adap with parameters:')
	for name in names:
	...
	if setting == 'seen':
		for attack_name, eps in attacks:
			...
			classifier_detector = BlackBoxClassifier(lambda x : f_tra(x, model, n_classes, best_tra_detector), input_shape = datashape[1:], nb_classes = n_classes + 1, clip_values = (0, 1))
			classifier_detector_attack = HopSkipJump(classifier_detector, verbose = False, targeted = True, batch_size = 1)
			print('\n------ Black box attack on classifier + transport detector', flush = True)
			adaptive1(classifier_detector, classifier_detector_attack, model, classifier, best_tra_detector, testloader, n_classes, maha_params, 1)
			
			classifier_detector = BlackBoxClassifier(lambda x : f_maha(x, model, n_classes, maha_params, best_maha_detector), input_shape = datashape[1:], nb_classes = n_classes + 1, clip_values = (0, 1))
			classifier_detector_attack = HopSkipJump(classifier_detector, verbose = False, targeted = True, batch_size = 1)
			print('\n------ Black box attack on classifier + mahalanobis detector', flush = True)
			adaptive1(classifier_detector, classifier_detector_attack, model, classifier, best_maha_detector, testloader, n_classes, maha_params, 1)
