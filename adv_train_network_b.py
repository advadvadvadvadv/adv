...
def train(model, optimizer, scheduler, criterion, trainloader, valloader, testloader, pnorm, lmt0, lml0, tau, uzs, con, nepochs, clip = 0):
    ...
    for e in range(nepochs):
        ...
        optimizer.zero_grad()
        vlo, vac = test(model, criterion, valloader)
        val_loss.append(vlo)
        val_acc.append(vac)
...
def experiment(dataset, modelname, pnorm, nfilters, learningrate, lambdatransport, lambdaloss0, tau, uzawasteps, batchnorm, bias, timestep, clip, classifier,
    ...
    train_loss, val_acc = train(model, optimizer, scheduler, criterion, trainloader, valloader, testloader, pnorm, lambdatransport, lambdaloss0, tau, uzawasteps, con, nepochs, clip)
